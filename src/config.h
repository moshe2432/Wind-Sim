#pragma once

#define N 64
#define SCREEN_WIDTH 1280       // Width of the window in pixels
#define SCREEN_HEIGHT 720       // Height of the window in pixels
#define GRID_GAP_WIDTH 320      // Width of the gap between the grid and the window edges in pixels
#define GRID_GAP_HEIGHT 40      // Height of the gap between the grid and the window edges in pixels
#define CELL_SIZE ((SCREEN_HEIGHT - 2 * GRID_GAP_HEIGHT) / (N + 2)) // Size of each cell in the grid in pixels

#define MOUSE_RADIUS 30         // Radius around the mouse position to affect the grid cells in pixels
#define MOUSE_FORCE 1.0f        // Force applied to the velocity field when the mouse is dragged

#define DENSITY_SCALE 5.0f      // Scale factor for the density field to control the visual representation of density
#define DENSITY_INJECTION_RATE 100.0f // Rate at which density is injected into the simulation